...
def thread_read(path_video: str, frame_queue: queue.Queue, event_stop: threading.Event):
...
def thread_write(length: int, fps: int, out_queue: queue.Queue, out_path: str):
...
def thread_safe_predict(frame_queue: queue.Queue, out_queue: queue.Queue, event_stop: threading.Event):
...
def main(arg):
    ...
    thread_read = threading.Thread(target=thread_read, args=(video_path, frame_queue, event_stop,))
    thread_read.start()

    thread_write = threading.Thread(target=thread_write, args=(length, fps, out_queue, arg.out,))
    thread_write.start()

    start_t = time.monotonic()
    for _ in range(arg.th):
        threads.append(threading.Thread(target=thread_safe_predict, args=(frame_queue, out_queue, event_stop,)))
